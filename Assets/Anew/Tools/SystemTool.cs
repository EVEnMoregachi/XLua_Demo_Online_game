...
bool  ifTouchOver  = true ;
 ...
 bool CheckGuiRaycastObjects()
	...
	{ 
        ...
        //射线碰到了物体
        if (Physics.Raycast(ray, out hit, 20000)) //1<<11 |   | (1 << 11) , (1 << 17)
        {
            Debug.Log(hit.collider.transform.name + "| " + hit.point.x);
           // LuaBehaviour.rayF(hit.collider.gameObject.name +"|" + hit.point.x+"|" + hit.point.z);
            LuaBehaviour.rayF(hit.collider.gameObject.name );
            return;
        }
