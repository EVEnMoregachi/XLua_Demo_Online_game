...
Shader "Game/WaterSample"
{ 
	Properties
	{
		_WaveScale ("Wave scale", Range (0.03,0.08)) = 0.063
		_ReflDistort ("Reflection distort", Range (0,3)) = 0.44
		_RefrDistort ("Refraction distort", Range (0,3)) = 0.40
		_FresnelBias ("Fresnel Bias(no use)", Range (0.0, 1.0)) = 0.0
		_FresnelScale ("Fresnel Scale(no use)", Range (1.0, 10.0)) = 3.0
		_FresnelPower ("Fresnel Power(no use)", Range (1.0, 5.0)) = 1.5
		_Fresnel ("Fresnel (A) ", 2D) = "gray" {}
		_WaterColorValue ("Water color value(RGB)", COLOR)  = ( .0, .0, .128, 1)
		_WaterColor ("Water color (RGB) and Alpha (A)(no use)", 2D) = "" {}
...
//#define WATER_REFLECTIVE 1
//#define WATER_REFRACTIVE 1
...
#include "UnityCG.cginc"
	...
	uniform float4 _WaterColorValue;
	...
	half4 frag( v2f i ) : COLOR
	{
		...
		half4 color = _WaterColorValue;//tex2D( _WaterColor, i.uv_WaterColor );
